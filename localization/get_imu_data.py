...
class GetImuData:
    ...
    def imu_integration(self):
        ...
        while True:
            ...
            # print('data',data)
            # 积分加速度计算速度
            acc_x = data[0]+1*math.sin(math.radians(data[7]))
            acc_y = data[1]-1*math.sin(math.radians(data[6]))
            print('acc_x',acc_x)
            print('acc_y',acc_y)
            acc_z = data[2]
            if abs(acc_x)<0.05:
            ...
            if theta_z<0.05:
                theta_z=0
            delta_speed_x = acc_x*time_gap
            delta_speed_y = acc_y*time_gap
            delta_speed_z = acc_z*time_gap
            speed_x += delta_speed_x
            speed_y += delta_speed_y
            speed_z += delta_speed_z
            speed = math.sqrt((speed_x**2+speed_y**2))
            print('speedx',speed_x)
            print('speedy',speed_y)
            print('theta_z',theta_z)

            self.out_q.put([speed,theta_z])
            time.sleep(0.005)

    def run(self):
        # 打印数据
        t1 = threading.Thread(target=self.get_data)
        t2 = threading.Thread(target=self.imu_integration)
...
def main():
    obj = GetImuData(port='com10', baud=115200)
    # 打印数据
