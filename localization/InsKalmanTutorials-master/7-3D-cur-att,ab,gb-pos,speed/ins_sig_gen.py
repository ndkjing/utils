...
def get_body_motion( psi0, theta0, gamma0, rot_changes_x, rot_changes_y, rot_changes_z, speed_changes, period ):
	...
	body_accel = []	
	for attitude, accel in zip( global_attitude, global_accel ):
		body_accel.append(
...
def get_imu_signal( body_accel, body_attitude_speed, acc_bias_std, acc_w_std, gyro_bias_std, gyro_w_std, period ):
	# Simulation time
	...
	imu_accel_bias_z = np.random.normal(0, acc_bias_std)
	imu_accel_bias = [
		np.matrix([
			[ imu_accel_bias_x ],
			[ imu_accel_bias_y ],
	...
	# Gyro bias
	imu_gyro_bias_x = np.random.normal(0, gyro_bias_std)
	imu_gyro_bias_y = np.random.normal(0, gyro_bias_std)
	imu_gyro_bias_z = np.random.normal(0, gyro_bias_std)
	imu_gyro_bias = [
		np.matrix([
			[ imu_gyro_bias_x ],
			[ imu_gyro_bias_y ],
...
def generate_signals(	
	...
	[ 
		...
		body_accel, body_attitude_speed, acc_bias_std, acc_w_std, gyro_bias_std, gyro_w_std, imu_period
	)
